...
public class DataStore<T> {
    ...
    private static class InMemoryDataStore {
        ...
        static synchronized DataStore<?> getInstance(final Class<?> clz,
                final boolean createNewInstance) throws DataStoreException {
    ...
    @SuppressWarnings("unchecked")
    public static <T> DataStore<T> createInMemory(final Class<T> clazz)
    ...
    @SuppressWarnings("unchecked")
    public static <T> DataStore<T> createInMemory(final Class<T> clazz,
            final boolean keepExisting) throws DataStoreException {
        return (DataStore<T>) InMemoryDataStore.getInstance(clazz,
                !keepExisting);
    }

    private DataStore(final Map<KeyElement, T> wrapStore, final Class<T> clz)
            throws DataStoreException {
    ...
    public T createInstance() {
        try {
            ...
            throw new AnnotationRuntimeException(
                    "Unable to create instance of class '" + dataTypeClass
                            + "'.", e);
    ...
    private T create(final T object, final KeyElement keyElement)
            throws DataStoreException {
        synchronized (dataStore) {
            if (keyElement.keyValuesMissing()
                    || dataStore.containsKey(keyElement)) {
    ...
    /**
     * Are the key values equal for both instances. If all compared key values
     * are <code>null</code> this also means equal.
     * 
     * @param first
     *            first instance to check for key equal
     * @param second
     *            second instance to check for key equal
    ...
    /**
     * Are the key values equal for both instances. If all compared key values
     * are <code>null</code> this also means equal. Before object (keys) are
     * compared it is validated that both object instance are NOT null and that
     * both are from the same class as this {@link DataStore} (see
     * {@link #dataTypeClass}). For the equal check on {@link #dataTypeClass}
     * instances without validation see {@link #isKeyEqual(Object, Object)}.
     * 
     * @param first
     *            first instance to check for key equal
     * @param second
     ...
     */
    @SuppressWarnings("unchecked")
    public boolean isKeyEqualChecked(final Object first, final Object second)
            throws DataStoreException {
        if (first == null || second == null) {
            ...
            throw new DataStoreException(
                    "First value is no instance from required class '"
                            + dataTypeClass + "'.");
        } else if (second.getClass() != dataTypeClass) {
            throw new DataStoreException(
                    "Second value is no instance from required class '"
                            + dataTypeClass + "'.");
        }
    ...
    private class KeyElement {
        ...
        private void addValue(final Object keyValue) {
            ...
            cachedHashCode = 89 * cachedHashCode
                    + (keyValue != null ? keyValue.hashCode() : 0);
        }
        ...
        public boolean equals(final Object obj) {
            ...
            if (this.keyValues != other.keyValues
                    && (this.keyValues == null || !this.keyValues
                            .equals(other.keyValues))) {
                return false;
            }
            return true;
        }
        ...
        public String toString() {
            return "KeyElement{" + "cachedHashCode=" + cachedHashCode
                    + ", keyValues=" + keyValues + '}';
        }
    }
    ...
    private class KeyAccess {
        ...
        KeyAccess(final Class<?> clazz) throws DataStoreException {
            keyFields = ANNOTATION_HELPER.getAnnotatedFields(clazz,
                    EdmKey.class);
            if (keyFields.isEmpty()) {
                throw new DataStoreException(
                        "No EdmKey annotated fields found for class " + clazz);
            }
        }
        ...
        private Object createKey(final Field field) {
            ...
            throw new UnsupportedOperationException(
                    "Automated key generation for type '" + type
    ...
    private KeyElement createSetAndGetKeys(final T object)
            throws DataStoreException {
