...
public class AnnotationHelper {
     ...
     * @return
     */
    public boolean keyMatch(final Object firstInstance,
            final Object secondInstance) {
        ...
        Map<String, Object> firstKeyFields = getValueForAnnotatedFields(
                firstInstance, EdmKey.class);
        Map<String, Object> secondKeyFields = getValueForAnnotatedFields(
                secondInstance, EdmKey.class);
        if (firstKeyFields.isEmpty() && secondKeyFields.isEmpty()) {
            throw new AnnotationRuntimeException(
                    "Both object instances does not have EdmKey fields defined ["
                            + "firstClass="
                            + firstInstance.getClass().getName()
                            + " secondClass="
                            + secondInstance.getClass().getName() + "].");
        }
     ...
     * @return
     */
    public boolean keyMatch(final Object instance,
            final Map<String, Object> keyName2Value) {
        Map<String, Object> instanceKeyFields = getValueForAnnotatedFields(
                instance, EdmKey.class);
    ...
    private boolean keyValuesMatch(final Map<String, Object> firstKeyValues,
        ...
        if (firstKeyValues.size() != secondKeyValues.size()) {
            ...
            throw new AnnotationRuntimeException(
                    "No keys given for key value matching.");
        } else {
    ...
    public String extractEntityTypeName(final EdmNavigationProperty enp,
            final Class<?> fallbackClass) {
        Class<?> entityTypeClass = enp.toType();
        return extractEntityTypeName(entityTypeClass == Object.class ? fallbackClass
                : entityTypeClass);
    }
    ...
    public String extractEntityTypeName(final EdmNavigationProperty enp,
        ...
        if (entityTypeClass == Object.class) {
            ...
            return extractEntityTypeName((toClass.isArray()
                    || Collection.class.isAssignableFrom(toClass) ? (Class<?>) ((ParameterizedType) field
                    .getGenericType()).getActualTypeArguments()[0] : toClass));
        } else {
     ...
     * the set entity type name is returned and if no name is set the default
     * name is generated from the simple class name.
     * 
    ...
    /**
     * Returns <code>NULL</code> if given class is not annotated. If annotated
     * the entity set name is returned and if no name is set a default name is
     * generated based on the simple class name.
     * 
     * @param annotatedClass
     * @return
     */
    public String extractEntitySetName(final Class<?> annotatedClass) {
        ...
        EdmEntitySet entitySet = annotatedClass
                .getAnnotation(EdmEntitySet.class);
        if (entitySet == null) {
            return null;
        }
    ...
    public FullQualifiedName extractEntityTypeFqn(final EdmEntityType type,
            final Class<?> annotatedClass) {
        if (type.namespace().isEmpty()) {
            return new FullQualifiedName(generateNamespace(annotatedClass),
                    extractEntityTypeName(annotatedClass));
        }
        return new FullQualifiedName(type.namespace(),
                extractEntityTypeName(annotatedClass));
    }
    ...
    public FullQualifiedName extractComplexTypeFqn(final Class<?> annotatedClass) {
        EdmComplexType type = annotatedClass
                .getAnnotation(EdmComplexType.class);
        if (type == null) {
            return null;
        }
    ...
    public FullQualifiedName extractComplexTypeFqn(final EdmComplexType type,
            final Class<?> annotatedClass) {
        if (type.namespace().isEmpty()) {
            return new FullQualifiedName(generateNamespace(annotatedClass),
     ...
     * 
     * @param <T>
     *            must be EdmEntityType or EdmComplexType annotation
     * @param annotatedClass
     * @param typeAnnotation
     * @return null if annotatedClass is not annotated or name set via
     *         annotation or generated via
     *         {@link #getCanonicalName(java.lang.Class)}
     */
    private <T extends Annotation> String extractTypeName(
            final Class<?> annotatedClass, final Class<T> typeAnnotation) {
        if (annotatedClass == Object.class) {
            return null;
        }
    ...
    /**
     * Get the set property name from an EdmProperty or EdmNavigationProperty
     * annotation.
     * 
     * @param field
     * @return
    ...
    public String getPropertyNameFromAnnotation(final Field field) {
        ...
        if (property == null) {
            EdmNavigationProperty navProperty = field
                    .getAnnotation(EdmNavigationProperty.class);
            if (navProperty == null) {
                throw new EdmAnnotationException(
                        "Given field '"
                                + field
    ...
    public String extractToRoleName(final EdmNavigationProperty enp,
            final Field field) {
    ...
    public String extractToRoleEntityName(final EdmNavigationProperty enp,
            final Field field) {
    ...
    public String getCanonicalRoleName(String name) {
        return "r_" + name.substring(0, 1).toUpperCase(Locale.ENGLISH)
                + name.substring(1);
    }

    public String extractRelationshipName(final EdmNavigationProperty enp,
            final Field field) {
    ...
    public EdmMultiplicity extractMultiplicity(final EdmNavigationProperty enp,
            final Field field) {
        EdmMultiplicity multiplicity = mapMultiplicity(enp.toMultiplicity());
        final boolean isCollectionType = field.getType().isArray()
                || Collection.class.isAssignableFrom(field.getType());
    ...
    /**
     * Set key fields based on values in map. If an key field is not available
     * or <code>NULL</code> in the map it will be not set as <code>NULL</code>
     * at the instance object.
     * 
     * @param instance
     * @param keys
     * @return
    ...
    public class AnnotatedNavInfo {
        ...
        public AnnotatedNavInfo(final Field fromField, final Field toField,
                final EdmNavigationProperty fromNavigation,
        ...
        public String getRelationshipName() {
            ...
            if (fromAssociation.isEmpty()
                    && fromAssociation.equals(toAssociation)) {
                return createCanonicalRelationshipName(getFromRoleName(),
                        getToRoleName());
            } else if (toAssociation.isEmpty()) {
                return fromAssociation;
            } else if (!toAssociation.equals(fromAssociation)) {
                throw new AnnotationRuntimeException(
                        "Invalid associations for navigation properties '"
                                + this.toString() + "'");
            }
        ...
        public String toString() {
            if (isBiDirectional()) {
                return "AnnotatedNavInfo{biDirectional = true" + ", toField="
                        + toField.getName() + ", toNavigation="
                        + toNavigation.name() + '}';
            }
            return "AnnotatedNavInfo{" + "fromField=" + fromField.getName()
                    + ", toField=" + toField.getName() + ", fromNavigation="
                    + fromNavigation.name() + ", toNavigation="
                    + toNavigation.name() + '}';
        }
    }

    public AnnotatedNavInfo getCommonNavigationInfo(final Class<?> sourceClass,
            final Class<?> targetClass) {
        List<Field> sourceFields = getAnnotatedFields(sourceClass,
                EdmNavigationProperty.class);
        List<Field> targetFields = getAnnotatedFields(targetClass,
                EdmNavigationProperty.class);

        if (sourceClass == targetClass) {
            // special case, actual handled as bi-directional
            return getCommonNavigationInfoBiDirectional(sourceClass,
                    targetClass);
        }
        ...
        for (Field sourceField : sourceFields) {
            if (ClassHelper.getFieldType(sourceField) == targetClass) {
                final EdmNavigationProperty sourceNav = sourceField
                        .getAnnotation(EdmNavigationProperty.class);
                final String sourceAssociation = extractRelationshipName(
                        sourceNav, sourceField);
                for (Field targetField : targetFields) {
                    if (ClassHelper.getFieldType(targetField) == sourceClass) {
                        final EdmNavigationProperty targetNav = targetField
                                .getAnnotation(EdmNavigationProperty.class);
                        final String targetAssociation = extractRelationshipName(
                                targetNav, targetField);
                        if (sourceAssociation.equals(targetAssociation)) {
                            return new AnnotatedNavInfo(sourceField,
                                    targetField, sourceNav, targetNav);
                        }
                    }
                }
            }
    ...
    private AnnotatedNavInfo getCommonNavigationInfoBiDirectional(
        ...
        for (Field sourceField : sourceFields) {
            final EdmNavigationProperty sourceNav = sourceField
                    .getAnnotation(EdmNavigationProperty.class);
            final String navTargetEntityName = extractEntityTypeName(sourceNav,
                    sourceField);
    ...
    public Class<?> getFieldTypeForProperty(final Class<?> clazz,
            final String propertyName) throws ODataAnnotationException {
        if (clazz == null) {
            return null;
        }
        ...
        if (field == null) {
            throw new ODataAnnotationException("No field for property '"
                    + propertyName + "' found at class '" + clazz + "'.");
        }
        return field.getType();
    }

    public Class<?> getFieldTypeForProperty(final Object instance,
            final String propertyName) throws ODataAnnotationException {
        if (instance == null) {
            return null;
        }
    ...
    public Object getValueForProperty(final Object instance,
            final String propertyName) throws ODataAnnotationException {
        if (instance == null) {
            return null;
        }

        Field field = getFieldForPropertyName(propertyName,
                instance.getClass(), true);
        if (field == null) {
            throw new ODataAnnotationException("No field for property '"
                    + propertyName + "' found at class '" + instance.getClass()
                    + "'.");
        }
        return getFieldValue(instance, field);
    }

    public void setValueForProperty(final Object instance,
        ...
        for (Field field : fields) {
            ...
            if (property != null) {
                if (property.name().isEmpty()
                        && getCanonicalName(field).equals(propertyName)) {
                    return field;
    ...
    public Object getValueForField(final Object instance,
            final String fieldName, final Class<? extends Annotation> annotation) {
        if (instance == null) {
            return null;
        }
        return getValueForField(instance, fieldName, instance.getClass(),
                annotation, true);
    }
    ...
    private Object getValueForField(final Object instance,
            final Class<?> resultClass,
            final Class<? extends Annotation> annotation,
            final boolean inherited) {
        return getValueForField(instance, null, resultClass, annotation,
                inherited);
    }

    public Map<String, Object> getValueForAnnotatedFields(
            final Object instance, final Class<? extends Annotation> annotation) {
        return getValueForAnnotatedFields(instance, instance.getClass(),
                annotation, true);
    }

    private Map<String, Object> getValueForAnnotatedFields(
            final Object instance, final Class<?> resultClass,
            final Class<? extends Annotation> annotation,
            final boolean inherited) {
        if (instance == null) {
            return null;
        ...
        Class<?> superClass = resultClass.getSuperclass();
        if (inherited && superClass != Object.class) {
            Map<String, Object> tmp = getValueForAnnotatedFields(instance,
                    superClass, annotation, true);
    ...
    public void setValueForAnnotatedField(final Object instance,
            final Class<? extends Annotation> annotation, final Object value)
        ...
        if (fields.isEmpty()) {
            throw new ODataAnnotationException(
                    "No field found for annotation '" + annotation
                            + "' on instance '" + instance + "'.");
        } else if (fields.size() > 1) {
            throw new ODataAnnotationException(
                    "More then one field found for annotation '" + annotation
                            + "' on instance '" + instance + "'.");
        }
    ...
    public List<Field> getAnnotatedFields(final Object instance,
            final Class<? extends Annotation> annotation) {
        if (instance == null) {
            return null;
        }
    ...
    public List<Field> getAnnotatedFields(final Class<?> fieldClass,
            final Class<? extends Annotation> annotation) {
    ...
    private Object getValueForField(final Object instance,
            final String fieldName, final Class<?> resultClass,
            final Class<? extends Annotation> annotation,
            final boolean inherited) {
        if (instance == null) {
            return null;
        ...
        Class<?> superClass = resultClass.getSuperclass();
        if (inherited && superClass != Object.class) {
            return getValueForField(instance, fieldName, superClass,
                    annotation, true);
        }
    ...
    private void setFieldValue(final Object instance, final Field field,
            final Object value) {
        try {
    ...
    private Object convert(final Field field, final String propertyValue) {
        ...
        try {
            ...
            throw new AnnotationRuntimeException(
                    "Conversion failed for string property [" + propertyValue
                            + "] on field [" + field + "] with error: "
                            + ex.getMessage(), ex);
        }
    }
    ...
    public boolean isEdmAnnotated(final Class<?> clazz) {
        if (clazz == null) {
            ...
            final boolean isEntity = null != clazz
                    .getAnnotation(EdmEntityType.class);
            final boolean isEntitySet = null != clazz
                    .getAnnotation(EdmEntitySet.class);
            final boolean isComplexEntity = null != clazz
                    .getAnnotation(EdmComplexType.class);
    ...
    private String firstCharToUpperCase(final String content) {
        ...
        return content.substring(0, 1).toUpperCase(Locale.ENGLISH)
                + content.substring(1);
    }

    public EdmSimpleTypeKind mapTypeKind(
            final org.apache.olingo.odata2.api.annotation.edm.EdmType type) {
        switch (type) {
        case BINARY:
    ...
    public EdmMultiplicity mapMultiplicity(final Multiplicity multiplicity) {
        ...
        case MANY:
            return EdmMultiplicity.MANY;
        default:
            throw new AnnotationRuntimeException("Unknown type '"
                    + multiplicity + "' for mapping to EdmMultiplicity.");
        }
    }
